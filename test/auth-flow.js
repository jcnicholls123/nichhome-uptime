...
(async () => {
  try {
    await waitForServer();
    assert.deepEqual(await (await request("/api/version")).json(), { name: "NichHome Uptime", version: "1.0.0-beta.33", channel: "beta" });
    ...
    assert.equal(adminSettings.app.version, "1.0.0-beta.33");
